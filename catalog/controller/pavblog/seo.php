 ...
 */
		 ...
		 */
		 public function index() {
			...
			if (isset($this->request->get['_route_'])) {
				...
				//begin seo-search
				if(strpos($parts[0], 'search') !== false ) {

					$forward = $this->_search($parts);

					if($forward)
						return $forward;
					else $this->request->get['route'] = array();
				}
				//end seo-search
				...
				foreach ($parts as $part) {
					...
					if ($query->num_rows) {
						...
						if ($url[0] == 'information_contact') {
							$this->request->get['information_contact'] = $url[0];
						}
				...
				}elseif (isset($this->request->get['information_contact'])) {
	...
	private function _search($parts) {
        ...
        if($parts){
            foreach($parts as $part){
                if((int)$part){
                    $this->request->get['category_id'] = $part;
                }elseif((string)$part == 'sub'){
                    $this->request->get['sub_category'] = true;
                }elseif((string)$part == 'description'){
