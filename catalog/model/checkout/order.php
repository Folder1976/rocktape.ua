...
// SMS
			...
			$day = date('w' , $now);
			$hour = date('H' , $now);
			$config_day = $this->config->get('config_day'.$day);
			$hours = explode('-', $config_day);
			$before = $hours[0] ? $hours[0] : false;
			$after  = $hours[1] ? $hours[1] : false;
			if(isset($hours[1])) $after = $hours[1];
			$message = $this->config->get('config_sms_text');
			if($hour < $before ||  $hour >=	$after) {
				$message = $this->config->get('config_sms_text_off');
				$send = true;
			}
			$off = $this->config->get('config_day'.$day.'_off');
			if($off || $day == '6' && $after >= $hour) {
				$message = $this->config->get('config_sms_sat_off');
				$send = true;
			}
			
			if($send) $res = file_get_contents('https://smsc.ua/sys/send.php?login=KinesioUA&psw=kinesio2018&phones='.$order_info['telephone'].'&mes='.urlencode(html_entity_decode(str_replace('\n', "\n", $message), ENT_QUOTES, 'UTF-8')).'&sender=RockTape&charset=utf-8');
